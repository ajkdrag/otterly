...
pub fn open_nlp_db(vault_root: &Path) -> Result<Connection, String> {
    ...
    let db_path = db_dir.join("nlp_cache.db");
...
pub fn get_cached(conn: &Connection, path: &str, content_hash: &str) -> Option<NlpAnalysis> {
    let mut stmt = conn
        .prepare("SELECT analysis_json FROM nlp_cache WHERE path = ?1 AND content_hash = ?2")
        .ok()?;
...
pub fn store_cached(
    ...
    let now = std::time::SystemTime::now()
        ...
        .as_secs() as i64;
    conn.execute(
        "INSERT OR REPLACE INTO nlp_cache (path, content_hash, analysis_json, analyzed_at) VALUES (?1, ?2, ?3, ?4)",
