...
export function resolve_editor_sync_open(input: {
  ...
  return (
    input.open_vault_id !== input.last_vault_id ||
...
export function resolve_editor_sync_restore_policy(input: {
  ...
  if (input.open_vault_id !== input.last_vault_id) {
    return "reuse_cache";
  }
